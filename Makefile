...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live ntfy server)"
...
test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/ntfy_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/ntfy_blade_mcp
